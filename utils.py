...
def auroc(causal_strength,causal_graph,causal_input,causal_label,causal_label_mask):
    ...
    causal_strength_fordis=torch.nn.Softmax(dim=-1)(causal_strength)
    causal_strength_fordis=torch.where(causal_strength_fordis>0.25,1,0).float()
    causal_strength=torch.masked_select(causal_strength,batch_adj_mask).detach().cpu().numpy()
    causal_strength_fordis_mask=torch.masked_select(causal_strength_fordis,batch_adj_mask).detach().cpu().numpy()
    ...
    fpr_in,tpr_in,thresholds_in=roc_curve(causal_strength_fordis_mask,causal_graph,pos_label=1)
    auc_in=auc(fpr_in,tpr_in)
    if fpr_in.all==tpr_in.all:
        auc_in=1
    return auc_s,auc_g,auc_in

def mse(causal_strength,causal_graph,causal_input,causal_label,causal_label_mask):
    batch_adj_mask=causal_label_mask.ge(0.5)
    causal_strength=torch.where(causal_strength>0.5,1,0).float()
    ...
    mse_in=crie(causal_graph,causal_strength)
