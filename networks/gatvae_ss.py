...
class gat_vae_do(nn.Module):
    ...
    def forward(self, doc_sents_h, doc_len, adj):
        ...
        X_raw,X_do,correlation_label,e,s,e_do,s_do=self.decoder(fU,doc_len,adj,I_A)#z=W(the inverse of I-A),causal graph is A
        causal_graph=I-I_A
        
        return X_raw,X_do,correlation_label,causal_graph,e,s,e_do,s_do
...
class GATVAEdecoder(nn.Module):
    ...
    def forward(self, doc_sents_h, doc_len, adj,I_A):
        ...
        correlation_label,W_do,do_num,e_do,s_do=self.computing(max_doc_len,causal_graph)
        ...
        return doc_sents_h_raw,doc_sents_h_do,correlation_label,e,s,e_do,s_do#
    ...
    def computing(self,max_doc_len,causal_graph):
        ...
        I_A_do=torch.inverse(causal_graph_do).unsqueeze(1)
        adj_e=torch.clone(I_A_do).reshape(-1,1)
        adj_s=torch.clone(I_A_do).reshape(-1,1)
        e=self.fce(adj_e).view(I_A_do.size()[0],I_A_do.size()[1],I_A_do.size()[2],-1)
        s=self.fcs(adj_s).view(I_A_do.size()[0],I_A_do.size()[1],I_A_do.size()[2],-1)
        z=self.sampling(e,s).squeeze(-1)
        z=torch.where(I_A_do==0,I_A_do,z)#batch*do_num,node_num,node_num
        return correlation_label,z,do_num,e,s
