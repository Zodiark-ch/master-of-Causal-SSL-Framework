...
from probabilistic_model import baseline_model,SS_model
from transformers import AdamW,get_cosine_schedule_with_warmup
from train_test import train_eval
from train_test_ssl import train_eval_ssl
...
parser.add_argument('--dataset',default='Causaction',type=str,help='Causaction or Causalogue')
...
parser.add_argument('--high_level_loss', type=str, default='loss2',  help='id of gpus')
parser.add_argument('--confounding', default='False', help='')
parser.add_argument('--bert_learning',default=False, help='')
...
def main(fold_id):
    ...
    if args.baseline=='False':
        model=SS_model(args,config=model_config).cuda()
    ...
    for epoch in range(1,int(args.epoch)+1):
        if args.baseline=='False':
            train_auroc_s,train_auroc_g,train_auroc_in,train_mse_s,train_mse_g,train_mse_in,train_distance=train_eval_ssl(model,train_loader, fold_id,epoch,args,optimizer,scheduler,logger,train=True)
            ...
            valid_auroc_s,valid_auroc_g,valid_auroc_in,valid_mse_s,valid_mse_g,valid_mse_in,valid_distance=train_eval_ssl(model,valid_loader, fold_id,epoch,args,optimizer,scheduler,logger,train=False)
            ...
            test_auroc_s,test_auroc_g,test_auroc_in,test_mse_s,test_mse_g,test_mse_in,test_distance=train_eval_ssl(model,test_loader, fold_id,epoch,args,optimizer,scheduler,logger,train=False)
        ...
        print('fold:{}  epoch:{}    valid_auroc_s:{:.3f}, valid_auroc_g:{:.3f}, valid_auroc_in:{:.3f}, test_auroc_s:{:.3f}, test_auroc_g:{:.3f}, test_auroc_in:{:.3f}, \
              valid_mse_s:{:.3f}, valid_mse_g:{:.3f}, valid_mse_in:{:.3f}, test_mse_s:{:.3f}, test_mse_g:{:.3f}, test_mse_in:{:.3f} '.format(fold_id, epoch, valid_auroc_s,valid_auroc_g,valid_auroc_in,test_auroc_s,test_auroc_g,test_auroc_in,valid_mse_s,valid_mse_g,valid_mse_in,test_mse_s,test_mse_g,test_mse_in))
...
if __name__=='__main__':
    ...
    wandb.init(config=wandb_config,reinit=True,project='SSF_framework',name='{}model_{}_lr_{}_batch_{}_fold_{}'.format(args.dataset,args.model_name,args.lr,args.batch_size,fold_id))
