...
def train_eval_ssl(model,dataloader,fold,epoch,args,optimizer,scheduler,logger,train=False):
    ...
    for train_step, batch in enumerate(dataloader, 1):
        ...
        if train:
            if args.dataset=='Causalogue':
                X_hat,X_hat_do,X,A,correlation_label,e,s,e_do,s_do,pred_results=model(batch_doc_len,batch_adj_mask,bert_token_b,bert_masks_b,bert_clause_b)
                
            if args.dataset=='Causaction':
                X_hat,X_hat_do,X,A,correlation_label,e,s,e_do,s_do,pred_results=model(batch_doc_len,batch_adj_mask,batch_cls,None,None)
        else:
            with torch.no_grad():
                if args.dataset=='Causalogue':
                    X_hat,X_hat_do,X,A,correlation_label,e,s,e_do,s_do,pred_results=model(batch_doc_len,batch_adj_mask,bert_token_b,bert_masks_b,bert_clause_b)
        ...
        loss_KL=model.loss_KL(e,s,e_do,s_do)
        loss_reconsctruction=model.loss_reconstruction(X_hat,X,batch_label_mask)
        loss_high_level=model.loss_hl(pred_results,A,batch_label,batch_label_mask)
        loss_ss=model.loss_ss(X_hat_do,correlation_label,batch_label_mask)
        #loss_KL=1
        #loss_re=1
        # loss_high_level=1
        # loss_KL=model.loss_KL(e,s)
        # loss_re=model.loss_reconstruction(X_rec,X)
        loss = loss_high_level + loss_KL + loss_reconsctruction+loss_ss
        if train:
            logger.info('TRAIN# fold: {}, epoch: {}, iter: {},  loss_high_level: {},  loss_KL: {},  loss_re:{}, loss_ss:{}'. \
                        format(fold,   epoch,    trainstep, loss_high_level,     loss_KL,     loss_reconsctruction,loss_ss))
            
            wandb.log({'epoch': epoch,  'trainstep':trainstep+len(dataloader)*epoch,'loss_all_train':loss,'loss_high_level_train':loss_high_level,'loss_KL_train':loss_KL,'loss_re_train':loss_reconsctruction,'loss_ss_train':loss_ss})
            ...
            if train_step % args.gradient_accumulation_steps == 0:
                ...
                scheduler.step()
                model.zero_grad()
            s_AUROC,g_AUROC,dis_AUROC=auroc(pred_results,A,batch_label,batch_label,batch_label_mask)
            s_MSE,g_MSE,dis_MSE=mse(pred_results,A,batch_label,batch_label,batch_label_mask)
            s_AUROC_all_train+=s_AUROC
            g_AUROC_all_train+=g_AUROC
            in_AUROC_all_train+=dis_AUROC
            s_AUROC=0
            g_AUROC=0
            dis_AUROC=0
