...
class SS_model(nn.Module):
    ...
    def forward(self,lengths,adj_mask,bert_token_b,bert_masks_b,bert_clause_b):
        ...
        H_raw,H_do,correlation_label,A,e,s,e_do,s_do=self.gnn(doc_sents_h,lengths,adj_mask)
        
        H_src=H_raw.unsqueeze(-2).expand(-1, -1, doc_sents_h.size()[1], -1)
        H_dst=H_raw.unsqueeze(-3).expand(-1, doc_sents_h.size()[1], -1, -1)
        H_pair=torch.cat((H_src,H_dst),dim=-1)
        pred_results=torch.relu(self.strengh(H_pair).squeeze(-1))
        
        
        H_do_src=H_do.unsqueeze(-2).expand(-1, -1, doc_sents_h.size()[1], -1)
        H_do_dst=H_do.unsqueeze(-3).expand(-1, doc_sents_h.size()[1], -1, -1)
        H_do_pair=torch.cat((H_do_src,H_do_dst),dim=-1)
        pred_results_do=torch.relu(self.strengh(H_do_pair).squeeze(-1))
        assert not torch.any(torch.isnan(pred_results))
        assert not torch.any(torch.isnan(A))
        
        return H_raw,H_do,doc_sents_h,A.squeeze(1),correlation_label,e,s,e_do,s_do,pred_results
    ...
    def loss_ss(self,H_do,correlation_label,batch_label_mask):
        ...
        raw_do=[]
        for i in range(0,max_doc_len-1):
            for j in range(i+1,max_doc_len):
                raw_do.append([i,j])#number=do_num
        for bc in range(batch):
            for do in range(do_num):
                batch_label_mask[bc*do_num+do][raw_do[do][0]]=0
                batch_label_mask[bc*do_num+do][raw_do[do][1]]=0
    ...
    def loss_hl(self,causal_strengh,causal_graph,batch_label,batch_label_mask):
        criterion = nn.BCEWithLogitsLoss(reduction='mean',pos_weight=torch.tensor([2]).cuda())
        batch_label_mask=batch_label_mask.ge(0.5)
        causal_strengh=torch.masked_select(causal_strengh,batch_label_mask)
        causal_graph=torch.masked_select(causal_graph,batch_label_mask)
        batch_label=torch.masked_select(batch_label,batch_label_mask)
    ...
    def loss_KL(self,e,s,e_do,s_do):
        # batch=e[1].size()[0]
        # utt=e[1].size()[1]
        # num=batch*utt*utt
        # sum=0
        ...
        # print(e,s)
        # print(e.pow(2),s.exp())
        KLD= -0.5 * torch.sum(1 + s_do - e_do.pow(2) - s_do.exp())
        sum2=KLD/num
        if sum1+sum2>4:
            return 1
        else:
    ...
    def loss_reconstruction(self,X_rec,X,batch_label_mask):
        
        
        
        batch_label_mask=batch_label_mask.ge(0.5)
        criterion = nn.BCEWithLogitsLoss(reduction='mean')
        assert X_rec.size()==X.size()
        batch,N,emb=X_rec.size()
        xhat_src=X_rec.unsqueeze(-2).expand(-1, -1, N, -1)
        xhat_dst=X_rec.unsqueeze(-3).expand(-1, N, -1, -1)
        x_src=X.unsqueeze(-2).expand(-1, -1, N, -1)
        x_dst=X.unsqueeze(-3).expand(-1, N, -1, -1)
        Sim_xhat=torch.cosine_similarity(xhat_src,xhat_dst,dim=-1)
        Sim_x=torch.cosine_similarity(x_src,x_dst,dim=-1)
